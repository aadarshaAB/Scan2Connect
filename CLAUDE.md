...
- `wifi/profile.py` — `build_profile_xml(creds: WifiCredentials) -> str`: WLAN_profile v1 XML for WPA2PSK/AES, WPA3SAE/AES, WPA/TKIP, open/none, WEP; `<nonBroadcast>` for hidden networks; XML-escapes SSID/key. WPA3SAE validated as a plain v1-namespace profile — no v3 namespace or `transitionMode` element needed (confirmed against `netsh wlan add profile`, which is stricter than XML well-formedness). Not wired into the app yet (E-09).
...
- WiFi features are Windows-only. Whether profile creation needs elevation is **unverified** — E-09 must test on a standard (non-admin) account. `netsh wlan add profile` (equivalent to `WlanSetProfile`) and `netsh wlan delete profile` both worked without elevation during E-08 validation on this machine, which is a good sign but not a substitute for E-09's explicit test.
...
**Current focus:** E-09 (not started) — Phase 1 (Windows 11 runs) in progress
**Next up:** E-09
