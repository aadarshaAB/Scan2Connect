...
- `wifi/connector.py` — `WifiConnector(QThread)`, now on the native WLAN API: builds a profile via `wifi/profile.py`, `WlanSetProfile`s it per-user (overwrites only the same-SSID profile, other saved profiles untouched), `WlanConnect`s, polls `current_connection` up to 15s. Signals unchanged: `status_updated(str)`, `connection_completed(bool, str)`. Constructor takes the full `WifiCredentials` object (not just ssid/password) so security type and hidden-network flag reach the profile builder — `ui/main_window.py`'s one call site was updated to match (`WifiConnector(creds)`). `pywifi`/`comtypes` fully removed.
...
**Current focus:** E-10 (not started) — Phase 1 (Windows 11 runs) in progress
**Next up:** E-10
